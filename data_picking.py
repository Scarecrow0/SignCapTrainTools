...
def picking_by_verify_model():
    ...
    with open(os.path.join(DATA_DIR_PATH, 'new_train_data_init'), 'rb') as f:
    ...
    while range_iter < len(all_data):
        ...
        for each in range(len(input_seg)):
            ...
            add_noise = True if random.random() > 0.85 else False
            if dis < threshold:
                # if dis > threshold and add_noise or dis < threshold:
    ...
    with open(os.path.join(DATA_DIR_PATH, 'new_train_data_picked_machine'), 'wb') as f:
